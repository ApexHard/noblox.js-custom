...
// Docs
/**
 * ✅ Obtenha o ID de um usuário a partir do seu nome de usuário.
 * @category User
 ...
 * @param {boolean} [bool=false] - (padrão: false), se for 'true' vai receber o objeto inteiro, se for 'false' vai receber só id do individuo
...
// Define
function getIdFromUsername(usernames, bool=false) {
  ...
  return http(httpOpt)
    .then(function (body) {
      ...
      results = results.map((result) => result !== undefined ? (bool ? result : result.id) : null)
...
exports.func = function (args) {
  ...
  return cache.wrap('IDFromName', usernames.map(username => username.toLowerCase()), function () {
    return getIdFromUsername(usernames, args?.boolean || false)
  })
}
